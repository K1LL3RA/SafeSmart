...
def ajustar_tamano_firma_preciso(ruta_imagen, max_pixels_x=0.007, max_pixels_y=0.003):
    """Ajusta el tamaño de la firma proporcionalmente respetando el máximo de píxeles."""
    emu_per_pixel = 9525
        ...
        for idx, i in enumerate(range(23, 43)):
            if idx < len(participantes):
                ...
                if carpeta_firmas:
                    ...
                    firma_insertada = False
                    for ext in extensiones:
                        ruta_firma = os.path.join(carpeta_firmas, f"{dni}{ext}")
                        if os.path.exists(ruta_firma):
                            firma = Image(ruta_firma)
                            if redimension_firma:
                                new_width, new_height = ajustar_tamano_firma_preciso(ruta_firma)
